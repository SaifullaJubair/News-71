    ...
    return (
        <div className='max-w-[1440px] w-[95%] mx-auto flex gap-6 mt-7'>
            ...
            <div className='flex-grow'>
                {
                    dislike ? <h2 className='title uppercase p-10 text-center mb-10 bg-purple-300 text-black text-2xl font-semibold'>All Dislike </h2> :
                        <Loader></Loader>
                }
                {
                    dislike?.length ? <p></p> :
                    ...
                    <Table.Body className="divide-y">
                        {
                            dislike?.map((comment, index) =>
                                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <Table.Cell>
                                        <Link className='text-blue-400 truncate ...' href={`/news/view/${comment?.id}`}> {comment?.heading?.slice(0, 40)}...</Link>
                                    </Table.Cell>
                                    <Table.Cell>
