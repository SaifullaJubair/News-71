        ...
        <div className="my-3">
          {comments?.map((comment) => (
            <div class="flex  border-b-2 pb-2 gap-3 my-3">
              <img
                class="inline-block h-12 w-12 rounded-full   border-blue "
                src={comment?.img}
              />
              <div>
                <h5 className=" text-gray-700 flex items-center gap-2 font-semibold">
                  {comment.username}
                </h5>
                <p className="text-sm ">{comment.comment}</p>
                <p className="text-sm ">{comment.createdAt?.slice(0, 10)}</p>
                <p className="text-sm py-2 text-gray-400">
                  Post:{comment.createdAt?.slice(11, 16)}
                </p>
              </div>
            </div>
          ))}
