    ...
    <div className="mt-16 mb-10 font-Times ">
      ...
      <section className="py-6 dark:bg-gray-800 dark:text-gray-50">
        <div className="grid grid-cols-1 gap-4 mx-auto md:grid-cols-4">
          {nextFourNews?.map((news) => (
            <Link href={`/news/view/${news?._id}`}>
              <div class=" w-full rounded overflow-hidden duration-300 hover:-translate-y-2 shadow-lg">
                <img src={news?.img} className="w-full h-44" alt="" />
                <div class="p-3">
                  <div class="font-bold text-xl mb-2">
                    {singleNews?.heading?.length > 43
                      ? `${singleNews?.heading.slice(0, 40)}...`
                      : singleNews?.heading}
                  </div>
                </div>
              </div>
          ...
          <Link
            href={`/news/view/${singleNews?._id}`}
            className="col-span-2 row-span-2 md:col-start-1 md:row-start-1 shadow-md "
          >
            <div class=" w-full rounded overflow-hidden duration-300 hover:-translate-y-2 ">
              <img src={singleNews?.img} className="w-full" alt="" />
              <div class="p-3">
